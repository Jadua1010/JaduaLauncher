...
public class XboxTokenAuthorizer {
	...
	public static XboxAuthorization authorizeWithXbox(String accessToken)
		...
		XboxAuthRequest<XblAuthProperties> xblPayload = new XboxAuthRequest<>(new XblAuthProperties("d=" + accessToken));
		...
		XboxAuthResponse xblResponse = HttpRequest.post(XBL_AUTHENTICATE_URL)
				...
				.expectResponseCodeOr(200, (req) -> {
					log.severe("Error authenticating with Xbox Live: " + Integer.toString(req.getResponseCode()));
		...
		XboxAuthRequest<XstsAuthProperties> xstsPayload = new XboxAuthRequest<>(
				new XstsAuthProperties(xblResponse.getToken()));
